...
// Parameters: num_epochs, local_size_x, local_size_y
int main(int argc, char *argv[]){
    ...
    int err;
    int global[2];
    global[0] = atoi(argv[2]);
    global[1] = atoi(argv[3]);
...
//    gradient = (float *) malloc (NUM_PIXELS*sizeof(float));
    ...
    err = clSetKernelArg(ko_vsqr, d_i++, sizeof(float) * global[1], NULL);
    if(err !=CL_SUCCESS) {
        printf("ERRO clSetKernelArg local_loss: %d\n", err);
    }
    err = clSetKernelArg(ko_vsqr, d_i++, sizeof(float) * global[0] * global[1], NULL);
    if(err !=CL_SUCCESS) {
        printf("ERRO clSetKernelArg dot_product: %d\n", err);
    }
